...
def generate_loop_simple(start_residue, end_residue):
    ...
    residue selector).'''
    loop = rosetta.protocols.loops.Loop(start_residue,
            end_residue)
    loop.set_cut((start_residue + end_residue)//2)
    return loop


def generate_loops_simple(pose, start_residue, end_residue):
    '''Function to get a loops object for LoopModeler. For now, simply
    do focus residue +/- 3 residues, but we can make this more
    sophisticated later (the constructor for Loops can also take a
    residue selector).'''
